...
namespace SchoolTests
{
    public class StudentControllerTests
    {
        ...
        public async Task GetStudents_ReturnOkResult_WithListOfStudents()
        {
            ...
            { new Student { StudentId = 1, Name = "Ana Pérez"} };
            var studentDtos = new List<StudentDto>
            { new StudentDto { StudentId = 1, Name = "Ana Pérez"} };

            _mockRepo.Setup(repo => repo.GetAllAsync(null)).ReturnsAsync(students);
            _mockMapper.Setup(mapper => mapper.Map
            <IEnumerable<StudentDto>>(It.IsAny<IEnumerable<Student>>()))
                .Returns(studentDtos);
