...
namespace School_API.Controllers
{
    ...
    public class StudentController : ControllerBase
    {
        ...
        public async Task<IActionResult> PutStudent(int id, StudentUpdateDto updateDto)
        {
            ...
            {
                return BadRequest("Los datos de entrada no son válidos o el ID del estudiante no coincide.");
            }
            ...
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    _logger.LogError($"Error de concurrencia al actualizar el estudiante con ID: {id}. " +
                        $"Detalles: {ex.Message}");
                    return StatusCode(StatusCodes.Status500InternalServerError,
        ...
        public async Task<IActionResult> PatchStudent(int id, JsonPatchDocument<StudentUpdateDto> patchDto)
        {
            ...
            try
            {
                ...
                using (var transaction = await _studentRepo.BeginTransactionAsync())
                {
                    ...
                    {
                        _logger.LogError($"Error al aplicar el parche al estudiante con ID {id}: {ex.Message}");
